...
Vertical slice landed in commit `2eff4b9` (frontend + tests), building on dust/03's backend stubs at `8452bf5`. `runUserSignedRecovery` rebuilds Merkle tree in-browser, generates Groth16 proof, asks `/mixer/withdraw` for unsigned tx where signer == origin wallet, user signs + broadcasts direct to RPC. `/relayer/*` never touched. Both load-bearing tests (backend throwing PrivacyAdapter + frontend `/relayer/*` 500 stub) green. State machine ownership stays single-owner via dust/03's `markWithdrawn`.

Caveat (release-note worthy): Positions created before this commit have no persisted `mixerWithdrawWitness` — UI falls back cleanly to legacy stealth-sweep for those.
