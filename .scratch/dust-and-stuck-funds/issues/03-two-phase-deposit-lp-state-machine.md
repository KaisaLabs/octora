...
Vertical slice landed in commit `8452bf5`. State-machine option (a) chosen: variants added to `ExecutionState` union guarded by existing `canTransition` table. Persisted intent stored before `DEPOSITED` transition. UI panels on Position detail + list. 15 new tests green; full backend suite 212/213 (1 pre-existing `buildWithdrawCloseIx` failure, unrelated). Web 30/30. Stub surface for dust/04 at `position.recover.service.ts` — exercised by the dust/04 commit.
