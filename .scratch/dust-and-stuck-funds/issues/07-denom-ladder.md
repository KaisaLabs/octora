...
Landed in commit `fc72e7e`. Multi-denom routing was further along than memory implied; this commit closes the remaining gaps (5 SOL added to bootstrap defaults, disabled-with-tooltip UI for thin pools, explicit per-bucket Anonymity Set Guard tests + `/mixer/pools` controller tests). Mainnet deploy is the only remaining manual step (operator-driven, out of scope).
