...
Rescoped AC verified met:

- `.out-of-scope/sweeper.md` updated w/ "Current status (2026-05-15)" section — scaffolds-only state of issues 05/06, ADR-0003 cited, protocol-level dust deferral acknowledged, band-aid argument restated, user-facing UX sweep distinguished from protocol sweeper
- `.ts.bak` claim refuted (`grep -ni "sweep" tests/octora-executor-happy-path.ts.bak` → 0 matches)
- Final grep run: 79 hits across `sweep|sweeper`, all categorized; 0 hits across `dust-collect|residual-collect`

**Suspect grep hits flagged for human (not auto-modified):**

- `octora-web/src/**` (~50 hits) — user-driven *manual* stealth→main sweep flow. Legitimate, terminology overlap only.
- `CORE_FEATURES_MVP_PLAN.md` (2) + `MVP_LAUNCH_PLAN.md` (1) — same UX, plan-level mentions.
- `runbooks/deployment/{key-rotation,secrets}.md` (2) — orthogonal context (key rotation, not dust).
- `runbooks/incident/mixer-pause.md` (1) — counterexample wording.

Rename candidate for the user-facing UX (`manual stealth withdrawal` or similar) noted, not done. Would clarify the terminology overlap. Not blocking.

Commit `a985078`. Closing as done.
