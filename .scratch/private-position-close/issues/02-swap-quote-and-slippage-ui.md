...
Landed in commit `b3b4c49`, merged via `c89b6bc`. Backend 279/279, web 66/66 post-merge, tsc clean. `CloseQuoteAdapter` interface mirrors close/01's `CloseOrchestrationAdapter` scope-down — production wiring of `buildSwapIx` + `getSwapQuote` remains the executor-surface gap noted in close/01's closure. `useCanClosePosition` hook (close/04) auto-wires to the `close-quote` response shape, so the v2 disabled-state on the Close button now lights up end-to-end without any further glue.
