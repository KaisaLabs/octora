...
**Production wiring status.** As of 2026-05-15 the three close adapters (`CloseOrchestrationAdapter`, `CloseQuoteAdapter`, `closeBuilderChain`) ship with production classes under `src/modules/positions/adapters/`. The seams are wired through `createApp` via `CreateAppOptions.{closeAdapter, closeQuoteAdapter, closeBuilderChain, closeBuilderLegBuilders}`. Each production class takes its live Executor / Mixer / DLMM dependencies at construct time and a per-position **context resolver** at call time; the resolver fields (stealth keypair, lb_pair, position pubkey, bin range, DLMM remaining-accounts) are not yet on `PositionRow`, so the production classes throw `CloseOrchestrationWiringIncompleteError` / `CloseQuoteWiringIncompleteError` (503-equivalent) when invoked without a resolver. The in-memory adapters used by tests stay in-place — they exercise the orchestrator's state-machine sequencing without RPC. Same precedent `OnchainMeteoraExecutor` set for the deposit-LP cluster; both context resolvers ship when the Position aggregate grows the stealth-context field set. ADR-0003 still applies — these are three serial txs, not an atomic compound. ADR-0004 still applies — no user-signed atomic deposit+addLP at current density.
